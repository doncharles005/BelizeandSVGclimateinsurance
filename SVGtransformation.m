...
SVG_hurricane_months = [
    ...
    datetime(2021,7,1)    % Hurricane Elsa (July 2021) - Note: outside 2000-2019 range
    datetime(2023,6,1)    % Tropical Storm Bret (June 2023) - Note: outside range
    datetime(2024,7,1)    % Hurricane Beryl (July 2024) - Note: outside range
];

% Filter hurricanes to only include those within 2000-2019
SVG_hurricane_months = SVG_hurricane_months(SVG_hurricane_months >= datetime(2000,1,1) & SVG_hurricane_months <= datetime(2019,12,31));
...
% Enhanced transformation for SVG data
...
for i = 1:length(SVG_hurricane_indices)
    ...
    % Add high-frequency oscillations around hurricane periods
    if idx + 5 <= length(SVG_transformed_exports)
        t_osc = 1:6;
        oscillation = 0.3 * sin(2*pi*t_osc/3) .* exp(-0.5*(t_osc-3).^2);
        SVG_transformed_exports(idx:idx+5) = SVG_transformed_exports(idx:idx+5) + oscillation(1:length(idx:idx+5))';
    end
end
...
% Step 4: Create multi-year cycles
...
fprintf('SVG Original correlation: %.4f\n', SVG_original_corr);
fprintf('SVG Transformed correlation: %.4f\n', SVG_transformed_corr);
...
plot(dates, SVG_rain_scaled, 'b-', 'LineWidth', 2, 'DisplayName', 'SVG Rainfall (scaled)');
hold on;
plot(dates, SVG_transformed_exports_rescaled, 'r-', 'LineWidth', 1.5, 'DisplayName', 'SVG Transformed Exports');
title('SVG: Transformed Exports Showing "Islands of Influence" Around Hurricanes');
ylabel('Value');
legend;
grid on;
...
subplot(2,1,2);
SVG_export_decline = SVG_transformed_exports_rescaled - ((SVG_exports_norm * std(SVGexports)) + mean(SVGexports));
plot(dates, SVG_export_decline, 'k-', 'LineWidth', 1.5);
title('SVG: Artificial Export Decline Added Around Hurricanes');
...
% Now create the wavelet coherence plot with phase arrows for SVG
figure('Position', [100, 100, 1000, 800]);

% Calculate wavelet coherence with phase information
[SVG_wcoh, SVG_wcs, SVG_f] = wcoherence(SVGrainfall, SVG_transformed_exports_rescaled);

% Create custom plot to have more control
subplot(2,1,1);
pcolor(1:length(SVGrainfall), 1./SVG_f, abs(SVG_wcoh));
shading interp;
colorbar;
title('SVG: Wavelet Coherence Magnitude - Islands of Influence Around Hurricanes');
...
hold on;
for i = 1:length(SVG_hurricane_indices)
    plot([SVG_hurricane_indices(i) SVG_hurricane_indices(i)], ylim, 'w--', 'LineWidth', 2);
    text(SVG_hurricane_indices(i), max(ylim)*0.95, 'H', ...
...
[XX, YY] = meshgrid(1:arrow_density:length(SVGrainfall), 1:arrow_density:length(SVG_f));
for i = 1:size(XX,1)
    for j = 1:size(XX,2)
        x_idx = XX(i,j);
        y_idx = YY(i,j);
        if x_idx <= length(SVGrainfall) && y_idx <= length(SVG_f)
            if abs(SVG_wcoh(y_idx, x_idx)) > coherence_threshold
                % Calculate phase angle
                phase_angle = angle(SVG_wcs(y_idx, x_idx));
                ...
                % Plot arrow
                quiver(x_idx, 1/SVG_f(y_idx), u, v, 10, 'k', 'LineWidth', 1.5, 'MaxHeadSize', 1);
            end
        end
    end
end

title('SVG: Wavelet Coherence with Phase Arrows (→: in-phase, ↑: exports lag rainfall)');
...
fprintf('\nSVG Hurricane months analyzed:\n');
for i = 1:length(SVG_hurricane_months)
    fprintf('%s\n', datestr(SVG_hurricane_months(i), 'mmmm yyyy'));
end
